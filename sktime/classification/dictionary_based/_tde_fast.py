...
__all__ = ["FastTemporalDictionaryEnsemble", "IndividualTDE", "histogram_intersection"]
...
class FastTemporalDictionaryEnsemble(BaseClassifier):
    ...
    predict uses 1 nearest neighbour with a histogram intersection
    distance function.
    ...
    def __init__(
        ...
        super(FastTemporalDictionaryEnsemble, self).__init__()
...
class IndividualTDE(BaseClassifier):
    ...
    See Also
    --------
    FastTemporalDictinaryEnsemble, SFA
    ...
    def _fit(self, X, y):
        ...
        self._transformers.append(
            SFAFast(
                ...
                remove_repeat_words=True if self.levels == 1 else False,
                lower_bounding=False,
                save_words=True if self.levels > 1 else False,  # only needed for levels
                n_jobs=self._threads_to_use,
            )
            # SFA(
            #     word_length=self.word_length,
        ...
        bag = self._transformers[0].fit_transform(X, y)

        # adding pyramids from raw words
        if self.levels > 1:
            bag = self._add_to_pyramid()

        self._transformed_data = bag

    def _add_to_pyramid(self):
        SFA = self._transformers[0]
        words = SFA.words

        sfa_words = []
        for lev in range(1, self.levels):
            # number of quadrants equal to pow(2, level-1)
            level = pow(2, lev - 1)
            for i in range(0, level):
                quadrant_words = words[
                    :,
                    int(i * words.shape[-1] / level) : int(
                        (i + 1) * words.shape[-1] / level
                    ),
                ]

                if quadrant_words.shape[-1] > 0:
                    sfa_words.append(
                        SFA.transform_to_bag(
                            quadrant_words, SFA.word_length_actual, None
                        )
                    )
    ...
    def _predict(self, X):
        ...
        # adding pyramids from raw words
        if self.levels > 1:
            test_bags = self._add_to_pyramid()
...
def histogram_intersection(first, second):
    ...
    """
    if isinstance(first, csr_matrix):  # csr matrix
        return hist_intersection(first, second)
    else:  # numpy array ?
        return np.sum(
            [
